...
public class Watch extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        DAO dao = new DAO();
        String idFilm_raw = request.getParameter("idFilm");
        if (idFilm_raw == null) {
            response.sendRedirect("browse");
        }
        try {
            int idFilm = Integer.parseInt(idFilm_raw);
            Film filmWatch = dao.getFilmById(idFilm);
            if (filmWatch == null) {
                response.sendRedirect("browse");
            }
            User user = new User();
            ...
            if(dao.insertFilmToHistoryWatch(idFilm, user.getIdUser()))
            request.setAttribute("idList", dao.getListIdByIdUser(user.getIdUser()));
            request.setAttribute("film", filmWatch);
        } catch (Exception e) {
            response.sendRedirect("browse");
        }
        request.getRequestDispatcher("Movies/watch.jsp").forward(request, response);
    }
